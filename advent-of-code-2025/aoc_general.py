...
def read_input_day(day, clean_whitespace=True) -> str:
    ...
    if input_cache_file.exists():
        ...
        req = requests.get(f"https://adventofcode.com/2025/day/{day}/input", cookies={ "session": dotenv["AOC_SESSION_COOKIE"] })
        input_text = req.text
        # Cache input
...
def submit_result_day(day, part, answer, allow_zero=False, allow_negative=False, allow_non_numeric=False, time_solve=False):
    ...
    req = requests.post(f"https://adventofcode.com/2025/day/{day}/answer", cookies={ "session": dotenv["AOC_SESSION_COOKIE"] }, data={ "level": part, "answer": answer })
    response = req.text
