...
int checkOptionCount(DOptionsTree *tree) {
    if (tree == NULL) return 0;
    int total = 0;
    for (int d = 0; d < SECRET_RADIX; d++) {
        if (tree->options[d] == NULL) continue;
        if (tree->options[d]->leaf) total += 1;
        total += checkOptionCount(tree->options[d]);
    }
    if (tree != &NO_OPTS_LEAF && total != tree->optionCount) {
        fatal("Option count wrong... Was %ld, but total %d",
            tree->optionCount, total);
    }
    return total;
}

void printOptionsTree(DOptionsTree *tree, int depth) {
    debug("DOT[%ld - %d](", tree->optionCount, checkOptionCount(tree));
    if (tree == NULL) {
        debug(" NULL )");
        return;
    }
...
void setDOpts(DOptionsTree *parent, digit d, DOptionsTree *newv) {
    if (parent->options[d] != NULL) {
        parent->optionCount -= parent->options[d]->optionCount;
    }
    if (newv != NULL) {
        newv->referencedBy++;
        parent->optionCount += newv->optionCount;
    }
...
DOptionsTree *decouple(DOptionsTree *parent, digit d) {
    ...
    parent->optionCount -= child->optionCount;
...
DOptionsTree *mergeOptionTrees(DOptionsTree *toTree, DOptionsTree *other,
    ...
    if (toTree->optionCount == 0) return NULL;
    return toTree;
}
...
DOptionsTree *optionsWithPredicate(Predicate predicate, int at) {
    ...
    if (at == 0)
        debug("Options assuming wrong: %d\n", checkOptionCount(putIn));
...
int main(int argc, char **argv) {
    ...
    while (true) {
        ...
        int optionsLeft = checkOptionCount(options);
